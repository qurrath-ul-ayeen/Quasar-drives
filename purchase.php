...
if (isset($_POST['buy'])) {
    ...
    (car_id, customer_name, customer_email, customer_phone, address,
     ...
     total_amount, purchase_status, loan_status)
    VALUES
    ('$car_id','$customer_name','$customer_email','$customer_phone','$address',
     '$payment_method','$bank_name','$account_holder_name','$ifsc_code','$transaction_id',
     '$car_price','Pending','No')";
...
if (isset($_POST['apply_loan'])) {
    ...
    VALUES
    ('$car_id','$customer_name','$customer_email','$customer_phone','$address',
     '$car_price','Pending','Yes')");

    $purchase_id = mysqli_insert_id($conn);

    // 2️⃣ Create loan
    mysqli_query($conn, "INSERT INTO loans
    (car_id, customer_name, customer_email, customer_phone, address,
     down_payment, loan_amount, interest_rate, loan_months,
     emi, total_payable, loan_status)
    VALUES
    ('$car_id','$customer_name','$customer_email','$customer_phone','$address',
     '$down_payment','$loan_amount','$interest_rate','$loan_months',
     '$emi','$total_payable','Pending')");

    $loan_id = mysqli_insert_id($conn);

    // 3️⃣ Link loan to purchase
    mysqli_query($conn,
        "UPDATE purchases SET loan_id='$loan_id' WHERE id='$purchase_id'"
    );

    echo "<script>alert('Loan Applied & Purchase Created Successfully');</script>";
}
?>
